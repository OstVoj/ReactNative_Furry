...
export class ParallaxPictureHeader extends React.PureComponent {
  ...
  _backgroundImageRef = null

  onBackgroundImageLoaded = () => {
    this.setState({ backgroundImageRef: findNodeHandle(this._backgroundImageRef) })
  }
  ...
  renderForeground = () => {
    const {
      foregroundText,
      foregroundContent = () => (<ForegroundText>{foregroundText}</ForegroundText>)
    } = this.props

    return (
      <ForegroundContainer>{foregroundContent()}</ForegroundContainer>
    )
  }
...
const CustomParallaxView = styled(ScrollIntoViewWrapper({
  animated: true,
  immediate: false,
  insets: {
    top: 0,
...
`
const ForegroundText = styled.Text.attrs({
  numberOfLines: 2,
  ellipsizeMode: 'tail'
})`
  color: #2b3032;
