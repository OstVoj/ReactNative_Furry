...
const mapStateToProps = state => ({
  ...
  requestOwnershipSelectorIsFetching: requestOwnershipSelector(state).isFetching,
...
export class PlaceReqOwnership extends React.PureComponent {
  ...
  renderRequestForm = () => {
    ...
    const thereAreMethods = !isFetching && isArray(methods) && methods.length > 0
    ...
    const normalizedMethods = showPicker ? _normalizeMethods(defaultItems.concat(methods)) : []
    ...
    return thereAreMethods ? (
      ...
      <ErrorMessage>Sorry, we can't confirm your ownership for this place.</ErrorMessage>
    )
  }
...
function _normalizeMethods (methods) {
  return methods
    .map(({ method, values }) => values.map(value => ({
      method,
      rawValue: value,
